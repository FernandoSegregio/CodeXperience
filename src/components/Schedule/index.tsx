...
const schedule: ScheduleItem[] = [
  ...
  { time: "10:40", room1: { theme: "Coffee Break", speaker: "" }, room2: { theme: "Coffee Break", speaker: "" } , room3: { theme: "Coffee Break", speaker: "" } },
  ...
  { time: "14:00", room1: { theme: "ignite talks", speaker: "" }, room2: { theme: "-", speaker: "" }, room3: { theme: "-" } },
  ...
  { time: "15:40", room1: { theme: "Coffee Break", speaker: "" }, room2: { theme: "Coffee Break", speaker: "" } , room3: { theme: "Coffee Break", speaker: "" } },
  ...
  { time: "16:50", room1: { theme: "Open Space", speaker: "" }, room2: { theme: "-", speaker: "" }, room3: { theme: "-" } },
  { time: "18:00", room1: { theme: "Final", speaker: "" }, room2: { theme: "-", speaker: "" }, room3: { theme: "-" } },
];
  ...
  return (
    <div className="bg-black p-4 text-white">
          ...
          <tbody>
            {schedule.map((item, index) => (
              <tr key={index} className={
                `${item.time === "12:30" || 
                item.time === "10:40" || 
                item.time === "15:40" ? "bg-gray-700" : "bg-black"}`}>
                <td className="px-4 py-2 border border-green-500 font-semibold">{item.time}</td>
                <td className="px-4 py-2 border border-green-500" colSpan={item.time === "12:30" ? 3 : 1}>
                  {item.room1 ? (
                    <>
      ...
      <div className="md:hidden space-y-4 max-w-[92%] m-auto">
        ...
        <div className="p-4 border border-green-500 rounded-lg bg-gray-800 mb-4">
          <p className="text-lg font-semibold">08:00 - Abertura</p>
        </div>
        {schedule.map((item, index) => (
          <div
            key={index}
            className={`p-4 border border-green-500 rounded-lg ${
              item.time === "12:30" ? "bg-gray-600" : "bg-gray-800"
            }`}
          >
            <p className="font-semibold">
              {item.time === "12:30" ? "12:30 Almoço" : item.time}
            </p>
            {item.time !== "12:30" && (
              <div className="space-y-2">
                <div>
                  <p className="font-bold">Sala 1</p>
                  {item.room1 && (
                    <>
                      <p className="text-white">{item.room1.theme}</p>
                      {item.room1.speaker && <p className="text-gray-400">{item.room1.speaker}</p>}
                    </>
                  )}
                </div>
                <div>
                  <p className="font-bold">Sala 2</p>
                  {item.room2 && (
                    <>
                      <p className="text-white">{item.room2.theme}</p>
                      <p className="text-gray-400">{item.room2.speaker}</p>
                    </>
                  )}
                </div>
                <div>
                  <p className="font-bold">Sala 3</p>
                  {item.room3 && (
                    <>
                      <p className="text-white">{item.room3.theme}</p>
                      <p className="text-gray-400">{item.room3.speaker}</p>
                    </>
                  )}
                </div>
